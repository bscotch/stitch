...
import {
  ...
  Gms2ResourceType,
} from './components/Gms2ResourceArray.js';
import { Gms2Object } from './components/resources/Gms2Object.js';
...
export interface Gms2MergerGitHubOptions
  extends StitchMergerOptions,
    Gms2GitHubRepoInfo {}

export class Gms2ProjectMerger {
  private options: StitchMergerOptions;

  constructor(
    private sourceProject: StitchProject,
    private targetProject: StitchProject,
  ...
  async merge() {
    const toImport = this.sourceProject.resources.filter((resource) =>
      this.resourceMatchesOptions(resource),
    );
    if (!this.options.skipDependencyCheck) {
      toImport.forEach((resource) =>
        this.assertAllDependenciesFound(resource, toImport),
      );
    }
  ...
  private resourcesMatch(
    resource1: Gms2ResourceSubclass,
    resource2: Gms2ResourceSubclass,
    requireSameFolder = false,
  ) {
    return (
  ...
  private assertAllDependenciesFound(
    ...
    allResources: Gms2ResourceSubclass[],
  ) {
    ...
    if (object.parentName) {
      const parentIsInModules = allResources.find(
        (resource) =>
          resource.type == 'objects' && resource.name == object.parentName,
      );
      assert(
        parentIsInModules,
        oneline`
    ...
    if (object.spriteName) {
      const spriteIsInModules = allResources.find(
        (resource) =>
          resource.type == 'sprites' && resource.name == object.spriteName,
      );
      assert(
        spriteIsInModules,
        oneline`
  ...
  private resourceMatchesOptions(
    resource: Gms2ResourceSubclass | Gms2IncludedFile,
  ) {
  ...
  private handleResourceConflict(
    targetResource: Gms2ResourceSubclass,
    toImport: Gms2ResourceSubclass[],
  ) {
    const isExtra =
      this.resourceMatchesOptions(targetResource) && // Would be imported if in source
      // But not in source
      !toImport.find((sourceResource) =>
        this.resourcesMatch(sourceResource, targetResource),
      );
  ...
  private importResource(sourceResource: Gms2ResourceSubclass) {
    ...
    const matchingTarget = this.targetProject.resources.find((targetResource) =>
      this.resourcesMatch(targetResource, sourceResource),
    );
    if (!matchingTarget) {
      this.cloneResourceFiles(sourceResource);
      this.targetProject.resources.register(
        sourceResource.toJSON(),
        this.targetProject.io,
      );
  ...
  private importIncludedFiles() {
    const sourceModuleFiles = this.sourceProject.includedFiles.filter((file) =>
      this.resourceMatchesOptions(file),
    );
    ...
    for (const sourceModuleFile of sourceModuleFiles) {
      let matchingTarget = this.targetProject.includedFiles.findByField(
        'name',
        sourceModuleFile.name,
      );
      if (!matchingTarget) {
        // Trim off the 'datafiles' parent folder
        const subdir = sourceModuleFile.folder;
        matchingTarget = this.targetProject.addIncludedFiles(
        ...
        this.resourceMatchesOptions(matchingTarget) ||
        this.options.onClobber == 'overwrite'
      ) {
        // Overwrite from source
        matchingTarget.replaceWithFileContent(
          sourceModuleFile.filePathAbsolute,
        );
    ...
    if (this.options.moveConflicting) {
      ...
      const localModuleFiles = this.targetProject.includedFiles.filter((file) =>
        this.resourceMatchesOptions(file),
      );
      const extraFiles = differenceBy(
        localModuleFiles,
        sourceModuleFiles,
        'name',
      );
  ...
  private cloneResourceFiles(sourceResource: Gms2ResourceBase) {
    ...
    const localYyDirAbsolute = paths.join(
      this.targetProject.storage.yypDirAbsolute,
      sourceResource.yyDirRelative,
    );
    this.targetProject.storage.copySync(
      sourceResource.yyDirAbsolute,
      localYyDirAbsolute,
    );
  }
}
